...
![Figure 11-1 M9301-YE Program Memory Map](figures/fig-11-1-m9301-ye-program-memory-map.png)
...
![Figure 11-2 Offset Switches and Corresponding Address Bits](figures/fig-11-2-offset-switches-address-bits.png)
...
## 11.5 DECnet Bootstrapping: DU11, DL11-E, DUP11
...
![Figure 11-3 M9301-YE Down Line Load Flowchart](figures/fig-11-3-m9301-ye-down-line-load-flowchart.png)
...
| Field | Description |
|-------|-------------|
| DEV TYPE (1 byte) | The device type at the requesting system. DL11-E = 4, DU11 = 2, and DUP11 = 12(8). |
...
| Boot Paper Tape Reader with Diag. | ON | ON | ON | ON | OFF | ON | ON | OFF | OFF | OFF | 106 |
...
| DECnet Boot DUP11 with Diag. | ON | ON | ON | ON | OFF | OFF | OFF | ON | OFF | ON | 164 |
| DECnet Boot DUP11 without Diag. | \* | ON | ON | ON | OFF | OFF | OFF | ON | OFF | OFF | 166 |
...
| Code | Function |
|------|----------|
| 165362 | Enter console emulator after running diagnostics |
| 165364 | To enter console emulator without running diagnostics |
...
The display routine is entered automatically. This routine will type the contents of R0, R4, R6, and R5 (note the sequence) on the Teletype or terminal in octal. Pressing the console boot switch causes the PDP-11/04 and PDP-11/34 systems to copy the PC into R5 before the power-up sequence starts. The console emulator (keyboard dispatch) program is then entered automatically. This routine types a carriage return, a line feed, and then the prompt symbol, $. The user can call bootstrap programs at this point by typing the appropriate device code. He then types an octal number following the device code specifying the drive number (default 0) and hits the carriage return key. Table 11-3 lists the peripheral bootstrap programs supported by the M9301-YE.
...
\*The # specifies the unit number (default 0). This may be from 0--17(8) for XU and XW and 0--36(8) for XL.
...
If the code 064 is placed in the microswitches, then the simulated trap through 24 is executed without running any diagnostics.

Figure 11-4 is an operator's flowchart showing the relation of the various options and codes available to the user.

![Figure 11-4 M9301-YE Operator's Flowchart](figures/fig-11-4-m9301-ye-operators-flowchart.png)
...
In order to avoid this incompatibility, the JMP (R)+ is programmed with (R) pointing back on the jump itself. On the 11/20, 11/05, 11/10, and 11/15 processors, execution of the instruction would cause (R) to be incremented to point to the following instruction, effectively continuing a normal execution sequence.
